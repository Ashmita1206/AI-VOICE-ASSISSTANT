...
from agentic.file_context_search.schemas import IndexedFile
...
def upsert_file(f: IndexedFile) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            f.entities_json, f.embedding_blob, time.time(),
        ),
    )
    conn.commit()
...
def get_file_by_path(path: str) -> Optional[IndexedFile]:
    ...
    row = conn.execute(
        ...
        "folder, folder_path, sample_text, keywords, summary, entities, embedding "
...
def _row_to_doc(row: tuple) -> IndexedFile:
    """Convert a DB row tuple to an IndexedFile."""
    return IndexedFile(
...
def get_all_files(with_embeddings: bool = True) -> List[IndexedFile]:
    ...
    if with_embeddings:
        ...
        query = (
            "SELECT path, filename, extension, size_bytes, modified_ts, "
            "folder, folder_path, sample_text, keywords, summary, entities, NULL "
            "FROM files"
        )
